...
export class PayrollController {
  ...
  @Post('generate')
  @DomainAudited()
  @ApiCreatedResponse({ description: 'تم إنشاء كشوف الرواتب بنجاح' })
  ...
  @Post(':id/approve')
  @DomainAudited()
  @ApiOkResponse({ description: 'تم اعتماد كشوف الرواتب' })
  ...
  @Delete(':id')
  @DomainAudited()
  @ApiOkResponse({ description: 'تم حذف مسودة الرواتب' })
  deleteDraft(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string) {
    return this.payrollService.deleteDraft(requireFarmId(user), id);
  }

  @Post(':id/pay')
  @DomainAudited()
  @ApiOkResponse({ description: 'تم صرف الرواتب' })
  ...
  @Get('periods')
  @ApiOkResponse({ description: 'قائمة فترات الرواتب' })
  ...
  @ApiOkResponse({ description: 'تفاصيل القسائم لفترة الراتب' })
