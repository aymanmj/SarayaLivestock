      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/60 dark:bg-slate-950/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white">استلام وتوريد شحنة أعلاف</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">{ingredient.name}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          <div className="p-3.5 bg-slate-50/60 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl flex justify-between">
            <span className="text-slate-500 dark:text-slate-400">الرصيد المتوفر حالياً:</span>
            <strong className="text-slate-900 dark:text-white font-bold">{Number(ingredient.currentStock).toLocaleString()} كجم</strong>
          </div>

          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">الكمية المستلمة (كجم) *</label>
            <input
              type="number"
              step="100"
              required
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-emerald-500 rounded-xl px-3.5 py-3 text-lg font-extrabold text-emerald-700 dark:text-emerald-400 focus:outline-none transition"
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">سعر الشراء الجديد (د.ل/كجم)</label>
              <input
                type="number"
                step="0.001"
                required
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-emerald-500 rounded-xl px-3.5 py-2.5 text-sm font-bold text-slate-900 dark:text-white focus:outline-none transition"
              />
            </div>
            <div>
              <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">إجمالي فاتورة الشحنة</label>
              <div className="p-2.5 bg-slate-50/80 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-bold text-emerald-700 dark:text-emerald-400 flex items-center justify-between">
