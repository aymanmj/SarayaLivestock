      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/60 dark:bg-slate-950/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white">تسجيل تشخيص وعلاج بيطري</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">تفعيل صمام الأمان وتتبع فترة تحريم اللحوم والألبان</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          ...
          {/* Tag Number */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">رقم قرط الحيوان المصاب *</label>
            <input
              type="text"
              required
              value={animalTag}
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-red-500 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none transition"
            />
          </div>

          {/* Diagnosis */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">التشخيص الطبي والحالة *</label>
            <input
              type="text"
              required
              value={diagnosis}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">اسم الدواء / المضاد *</label>
              <input
                type="text"
                required
                value={drugName}
            ...
            <div>
              <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">فترة التحريم (أيام) *</label>
              <input
                type="number"
                min="0"
                required
                ...
                className="w-full bg-slate-50 dark:bg-slate-950 border border-red-500/50 focus:border-red-500 rounded-xl px-3.5 py-2.5 text-sm font-bold text-red-400 focus:outline-none transition"
              />
            </div>
          </div>
          ...
          {/* Treated By */}
          <div>
            <label className="block font-semibold text-slate-700 dark:text-slate-300 mb-1.5">الطبيب البيطري المعالج</label>
            <input
              type="text"
              value={treatedBy}
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2.5 bg-red-600 hover:bg-red-500 text-slate-900 dark:text-white rounded-xl text-xs font-bold transition shadow-lg shadow-red-900/30 flex items-center gap-2 disabled:opacity-50"
            >
