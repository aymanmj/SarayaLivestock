...
export const AccountingGL: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-xl">
        <div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-slate-900 dark:text-white font-bold text-xs rounded-xl shadow-lg shadow-purple-600/20 flex items-center gap-2 transition"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-slate-900 dark:text-white font-bold text-xs rounded-xl shadow-lg shadow-emerald-600/30 flex items-center gap-2 transition"
          >
          ...
          <button
            onClick={fetchData}
            className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 transition border border-slate-300 dark:border-slate-700"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <div className="text-[10px] text-emerald-700 dark:text-emerald-400">يشمل قطعان الألبان والتسمين والمحلب</div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <div className="text-[10px] text-slate-500 dark:text-slate-400">مبيعات الحليب الخام + الماشية والسماد</div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <div className="text-[10px] text-slate-500 dark:text-slate-400">استهلاك الأعلاف + البيطرة والعمالة</div>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2">
          ...
          <div className={`text-xl font-black flex items-center gap-1.5 ${trialBalance?.isBalanced ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
          ...
          <div className="text-[10px] text-slate-500 dark:text-slate-400">إجمالي المدين = إجمالي الدائن</div>
        </div>
      </div>

      {/* Alerts */}
      {feedback && (
        <div className="p-3.5 rounded-xl bg-emerald-100 dark:bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-semibold flex items-center justify-between animate-in fade-in">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 shrink-0" />
            <span>{feedback}</span>
          </div>
          <button onClick={() => setFeedback(null)} className="text-emerald-700 dark:text-emerald-400 hover:text-emerald-200">✕</button>
        </div>
      )}

      {error && (
        <div className="p-3.5 rounded-xl bg-rose-100 dark:bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs font-semibold flex items-center justify-between animate-in fade-in">
          <div className="flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 shrink-0" />
            <span>{error}</span>
          </div>
          <button onClick={() => setError(null)} className="text-rose-600 dark:text-rose-400 hover:text-rose-200">✕</button>
        </div>
      )}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 border-b-2 text-xs font-bold transition shrink-0 ${
                isActive
                  ...
                  : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200 hover:border-slate-300 dark:border-slate-700'
              }`}
            >
      ...
      {activeTab === 'entries' && (
        <div className="space-y-4">
          <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl">
            <div className="p-4 bg-slate-850 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-500 dark:text-slate-400">إجمالي القيود: {formatNumber(journalEntries.length)} قيد</span>
            </div>

            <div className="overflow-x-auto">
              <table className="w-full text-right text-xs">
                <thead>
                  <tr className="bg-slate-50/60 dark:bg-slate-950/60 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                  {journalEntries.length === 0 ? (
                    ...
                    journalEntries.map((entry) => (
                      <tr key={entry.id} className="hover:bg-slate-100/40 dark:bg-slate-800/40 transition">
                        <td className="p-3.5 font-mono font-bold text-emerald-700 dark:text-emerald-400">{entry.entryNumber}</td>
                        <td className="p-3.5 text-slate-700 dark:text-slate-300 font-mono">
                          {formatDate(entry.entryDate)}
                        </td>
                        <td className="p-3.5 text-slate-900 dark:text-white font-semibold">{entry.description}</td>
                        <td className="p-3.5">
      ...
      {activeTab === 'accounts' && (
        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl space-y-4 p-5">
          <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
            <h3 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <FolderTree className="w-4 h-4 text-emerald-700 dark:text-emerald-400" />
              دليل شجرة الحسابات المالية المعتمدة (IAS 41)
            </h3>
            <span className="text-[11px] text-slate-500 dark:text-slate-400">إجمالي الحسابات: {formatNumber(accounts.length)}</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {accounts.map((acc) => (
              <div
                key={acc.id}
                className="p-4 rounded-2xl bg-slate-100/40 dark:bg-slate-800/40 border border-slate-300/60 dark:border-slate-700/60 flex items-center justify-between"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded font-mono font-bold text-xs bg-white dark:bg-slate-900 text-emerald-700 dark:text-emerald-400 border border-slate-300 dark:border-slate-700">
                      {acc.code}
                    </span>
                ...
                <div className="text-left font-mono font-bold text-sm text-emerald-700 dark:text-emerald-400">
            ...
            <table className="w-full text-right text-xs">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60">
                {trialBalance?.accounts?.map(row => (
                  <tr key={row.id} className="hover:bg-slate-100/30 dark:bg-slate-800/30">
                    <td className="p-3.5 font-mono text-emerald-700 dark:text-emerald-400 font-bold">{row.code}</td>
                    <td className="p-3.5 text-slate-900 dark:text-white font-semibold">{row.name}</td>
                    <td className="p-3.5 text-slate-500 dark:text-slate-400 text-[11px]">{row.category}</td>
                    <td className="p-3.5 font-mono text-slate-700 dark:text-slate-300">{formatNumber(row.totalDebit, 2)}</td>
                    <td className="p-3.5 font-mono text-slate-700 dark:text-slate-300">{formatNumber(row.totalCredit, 2)}</td>
              ...
              <tfoot>
                <tr className="bg-slate-50 dark:bg-slate-950 font-bold text-slate-900 dark:text-white border-t border-slate-300 dark:border-slate-700">
                  <td colSpan={3} className="p-3.5 text-emerald-700 dark:text-emerald-400">الإجمالي العام لميزان المراجعة</td>
                  ...
                  <td colSpan={2} className="p-3.5 text-center text-emerald-700 dark:text-emerald-400">✓ توازن كامل 100%</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center p-2 rounded-xl bg-slate-100/40 dark:bg-slate-800/40">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">إجمالي إيرادات الحليب والماشية:</span>
              ...
              <div className="flex justify-between items-center p-2 rounded-xl bg-slate-100/40 dark:bg-slate-800/40">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">إجمالي تكاليف الأعلاف والتشغيل:</span>
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-sm font-bold">
                <span className="text-slate-900 dark:text-white">صافي أرباح التشغيل للمزرعة:</span>
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">IAS 41</span>
            </div>

            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center p-2 rounded-xl bg-slate-100/40 dark:bg-slate-800/40">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">إجمالي الأصول البيولوجية والمتداولة:</span>
              ...
              <div className="flex justify-between items-center p-2 rounded-xl bg-slate-100/40 dark:bg-slate-800/40">
                <span className="text-slate-700 dark:text-slate-300 font-semibold">إجمالي الالتزامات وحقوق الملكية:</span>
                <span className="font-mono font-bold text-emerald-700 dark:text-emerald-400">
                  {formatMoney((balanceSheet?.totalLiabilities ?? 0) + (balanceSheet?.totalEquity ?? 0))}
                </span>
              </div>

              <div className={`pt-3 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-sm font-bold ${balanceSheet?.isBalanced ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
      ...
      {activeTab === 'years' && (
        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl p-5 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
            ...
            <button
              onClick={() => setIsRolloverModalOpen(true)}
              className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-slate-900 dark:text-white font-bold text-xs rounded-xl shadow-md transition flex items-center gap-2 self-start sm:self-auto"
            >
              <ArrowRightLeft className="w-4 h-4" />
          ...
          <div className="space-y-4">
            {fiscalYears.map((fy) => (
              <div key={fy.id} className="p-4 rounded-2xl bg-slate-100/50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-700 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-slate-900 dark:text-white">السنة المالية: {fy.yearName}</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                  {fy.periods?.map(p => (
                    <div
                      key={p.id}
                      className="p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs flex flex-col justify-between gap-1.5"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-800 dark:text-slate-200">{p.periodName}</span>
                        {p.status === 'CLOSED' ? (
                      ...
                      <div className="flex items-center justify-between text-[10px]">
                        <span className={p.status === 'CLOSED' ? 'text-rose-600 dark:text-rose-400 font-bold' : 'text-emerald-700 dark:text-emerald-400 font-bold'}>
                          {p.status === 'CLOSED' ? 'مقفل' : 'مفتوح'}
                        </span>
                        <button
                          onClick={() => handleTogglePeriod(p.id, p.status)}
                          className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 transition text-[10px]"
                        >
      ...
      {isRolloverModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-850">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <ArrowRightLeft className="w-4 h-4 text-purple-700 dark:text-purple-400" />
                إقفال السنة المالية وترحيل الأرصدة للسنة القادمة
              </h3>
              <button onClick={() => setIsRolloverModalOpen(false)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleExecuteRollover} className="p-6 space-y-4 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">السنة المالية المراد إقفالها</label>
                <select
                  value={selectedYearId}
                  onChange={e => setSelectedYearId(e.target.value)}
                  className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-2.5 text-white"
                  required
                >
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">اسم/رقم السنة المالية الجديدة</label>
                <input
                  type="text"
                  value={nextYearInput}
                  onChange={e => setNextYearInput(e.target.value)}
                  placeholder="2027"
                  className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-2.5 text-white font-mono"
                  required
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-slate-900 dark:text-white font-bold rounded-xl shadow-lg transition disabled:opacity-50"
                >
      ...
      {isEntryModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-850">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Plus className="w-4 h-4 text-emerald-700 dark:text-emerald-400" />
                تسجيل قيد يومية عام جديد (Double-Entry Journal)
              </h3>
              <button onClick={() => setIsEntryModalOpen(false)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">✕</button>
            </div>

            <form onSubmit={handleSaveEntry} className="p-6 overflow-y-auto space-y-4 flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">تاريخ القيد</label>
                  <input
                    type="date"
                    dir="ltr"
                    lang="en-CA"
                    ...
                    className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-2.5 text-slate-900 dark:text-white text-xs font-mono text-right"
                    required
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">البيان / الشرح</label>
                  <input
                    type="text"
                    value={entryDescription}
                    onChange={e => setEntryDescription(e.target.value)}
                    placeholder="مثال: شراء أعلاف مركزة / صرف أدوية / مبيعات حليب..."
                    className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-2.5 text-white text-xs"
                    required
                  />
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                  ...
                  {entryLines.map((line, idx) => (
                    <div key={idx} className="flex flex-wrap items-center gap-2 bg-slate-100/40 dark:bg-slate-800/40 p-2.5 rounded-xl border border-slate-300 dark:border-slate-700">
                      <select
                        ...
                        className="flex-1 bg-slate-850 border border-slate-300 dark:border-slate-700 rounded-lg p-2 text-slate-900 dark:text-white text-xs"
                        required
                      >
                      ...
                      <input
                        ...
                        className="w-28 bg-slate-850 border border-slate-300 dark:border-slate-700 rounded-lg p-2 text-slate-900 dark:text-white text-xs font-mono"
                      />
                      ...
                      <input
                        ...
                        className="w-36 bg-slate-850 border border-slate-300 dark:border-slate-700 rounded-lg p-2 text-slate-900 dark:text-white text-xs"
                      />
                      ...
                      {entryLines.length > 2 && (
                        <button
                          ...
                          className="p-1.5 rounded-lg text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:bg-rose-500/10"
                        >
                ...
                <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs font-mono font-bold">
                  <div className="flex items-center gap-4">
                    <span className="text-slate-500 dark:text-slate-400">إجمالي المدين: <strong className="text-slate-900 dark:text-white">{formatMoney(totalDebit)}</strong></span>
                    <span className="text-slate-500 dark:text-slate-400">إجمالي الدائن: <strong className="text-slate-900 dark:text-white">{formatMoney(totalCredit)}</strong></span>
                    <span className="text-slate-500 dark:text-slate-400">الفرق: <strong className={isBalanced ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}>{formatMoney(Math.abs(totalDebit - totalCredit))}</strong></span>
                  </div>
