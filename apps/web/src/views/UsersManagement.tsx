...
export const UsersManagement: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 text-xs font-bold flex items-center gap-2 transition"
          >
      ...
      {/* Feedback Alert */}
      {feedback && (
        <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-semibold flex items-center justify-between">
          <div className="flex items-center gap-2">
      ...
      {/* Add User Modal / Form */}
      {isAddingUser && (
        <div className="p-5 rounded-2xl bg-slate-900 border border-emerald-500/40 shadow-2xl animate-in fade-in duration-200">
          ...
          <form onSubmit={handleAddUserSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            ...
            <div>
              <label className="block text-slate-400 mb-1 font-semibold">الاسم ثلاثي</label>
              <input
                type="text"
                value={newFullName}
            ...
            <div>
              <label className="block text-slate-400 mb-1 font-semibold">الدور والصلاحية (Role)</label>
              <select
                value={newRole}
            ...
            <div>
              <label className="block text-slate-400 mb-1 font-semibold">كلمة المرور المبدئية</label>
              <input
                type="password"
                value={newPassword}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400">{users.length} مستخدم مسجل</span>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-right">
            <thead>
              <tr className="border-b border-slate-800 text-[11px] font-bold text-slate-400 bg-slate-950/40">
                ...
                <tr key={u.id} className="hover:bg-slate-800/40 transition">
                  ...
                  <td className="py-3 px-4">
                    <select
                      ...
                      className="bg-slate-800 border border-slate-700 rounded-lg px-2 py-1 text-slate-200 text-[11px] font-bold focus:outline-none focus:border-emerald-500"
                    >
                  ...
                  <td className="py-3 px-4">
                    {u.isActive ? (
                      ...
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-500/10 border border-rose-500/30 text-rose-400">
                        معطل مؤقتاً
                      </span>
                    )}
                  </td>
