...
import { Users, Plus, Shield, CheckCircle2, UserCheck, Stethoscope, BadgeDollarSign, Briefcase, Settings } from 'lucide-react';
import { generatedApiClient, unwrapGenerated, getJobTitles } from '../api/client';
...
export const HrEmployeesView: React.FC = () => {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const finalJobTitle = jobTitle === '__CUSTOM__' ? customJobTitle.trim() : jobTitle;
    if (!finalJobTitle) {
      setFeedback('يرجى تحديد أو إدخال المسمى الوظيفي');
      return;
    }
    ...
    try {
      ...
      setIsAdding(false);
      // reset
      setEmpCode(''); setFirstName(''); setLastName(''); setBaseSalary(''); setHireDate(''); setCustomJobTitle('');
      loadJobTitles();
    } catch (error: any) {
      setFeedback(error.message);
    }
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold flex items-center gap-1.5 border border-slate-200 dark:border-slate-700 transition"
          >
          ...
          <form onSubmit={handleAddSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div>
              ...
              <input
                ...
                required
                className="w-full p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 border-none text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 font-mono"
              />
            </div>
            <div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="font-bold text-slate-600 dark:text-slate-400">المسمى الوظيفي</label>
                <button
                  type="button"
                  onClick={() => setIsJobTitlesModalOpen(true)}
              ...
              >
                ...
                <option value="__CUSTOM__">+ إضافة مسمى وظيفي جديد...</option>
              </select>
              ...
              {jobTitle === '__CUSTOM__' && (
                <input
                  ...
                  placeholder="اكتب المسمى الوظيفي الجديد هنا"
                  required
                  autoFocus
                  className="w-full mt-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-emerald-500 text-slate-900 dark:text-white text-xs focus:ring-2 focus:ring-emerald-500"
                />
              )}
            </div>

            <div>
              <label className="block mb-1 font-bold text-slate-600 dark:text-slate-400">الراتب الأساسي (د.ل)</label>
              <input
                type="number"
                value={baseSalary}
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {employees.map(emp => (
              <tr key={emp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                <td className="p-4 font-mono font-bold text-slate-700 dark:text-slate-300">{emp.employeeCode}</td>
                ...
                <td className="p-4 font-mono text-emerald-600 dark:text-emerald-400 font-bold">
                  {Number(emp.baseSalary).toLocaleString(undefined, { minimumFractionDigits: 2 })} د.ل
                </td>
                <td className="p-4">
                  <span className={`px-2 py-1 rounded-lg text-[10px] font-bold ${emp.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400' : 'bg-slate-100 text-slate-500'}`}>
