    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">حساب فوري لمعدل النمو اليومي ومؤشرات كفاءة استهلاك العلف وجاهزية البيع</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <span className="text-xs text-slate-400 font-medium">متوسط الزيادة اليومية للقطيع (ADG)</span>
          ...
          <p className="text-[11px] text-slate-400 mt-2">محسوب من الحيوانات التي لديها وزنان مؤرخان على الأقل</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <span className="text-xs text-slate-400 font-medium">متوسط معامل التحويل الغذائي (FCR)</span>
          <div className="text-2xl font-extrabold text-purple-400 mt-1">
            غير متاح
          </div>
          <p className="text-[11px] text-slate-400 mt-2">يتطلب ربط استهلاك العلف بالحيوان أو مجموعة التسمين</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <span className="text-xs text-slate-400 font-medium">جاهزية البيع والذبح (وزن &gt; 480 كجم)</span>
          <div className="text-2xl font-extrabold text-emerald-400 mt-1">
            {formatNumber(readyForSale)} <span className="text-xs text-slate-400">رأس جاهزة</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-2">يعتمد على أحدث وزن مسجل لكل حيوان</p>
        </div>
      </div>

      {/* Main Table */}
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <h3 className="font-bold text-white text-sm">سجل أوزان دفعات وعجول التسمين</h3>
            ...
            <tbody className="divide-y divide-slate-800/60 text-xs">
              {data.map(row => (
                <tr key={row.id} className="hover:bg-slate-800/40 transition">
                  <td className="py-3 px-3 font-mono font-extrabold text-white text-sm">#{row.tagNumber}</td>
                  <td className="py-3 px-3">
                    <span className="font-medium text-slate-200 block">{row.breed}</span>
                    <span className="text-[10px] text-slate-500">{row.barn}</span>
                  </td>
                  <td className="py-3 px-3 text-slate-400">{row.entryWeight == null ? 'غير مسجل' : `${row.entryWeight} كجم`}</td>
                  <td className="py-3 px-3 font-bold text-purple-400">{row.currentWeight == null ? 'غير مسجل' : `${row.currentWeight} كجم`}</td>
                  <td className="py-3 px-3 font-bold text-emerald-400">{row.adgKg == null ? 'غير متاح' : `+${formatNumber(row.adgKg, 2)} كجم/يوم`}</td>
                  <td className="py-3 px-3 font-semibold text-slate-300">غير متاح</td>
                  <td className="py-3 px-3">
                    ...
                    {row.status === 'NO_DATA' && (
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-500/10 border border-slate-500/30 text-slate-400">
                        بيانات غير كافية
                      </span>
                    )}
                  </td>
                  <td className="py-3 px-3 text-left">
                    <button
                      ...
                      className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg font-bold text-[11px] transition border border-slate-700"
                    >
                      وزن جديد
                    </button>
                  </td>
