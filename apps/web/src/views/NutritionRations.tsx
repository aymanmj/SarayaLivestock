    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">محرك تركيب العلائق الاقتصادية، إدارة مخازن الخامات، وصرف الوجبات للحظائر</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-400 text-xs font-medium">إجمالي المخزون بالمستودع</span>
            <div className="p-2 bg-amber-500/10 text-amber-400 rounded-xl">
              <Archive className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-extrabold text-white">{totalStockTons}</span>
            <span className="text-xs text-slate-400">طن أعلاف</span>
          </div>
          <p className="text-[11px] text-emerald-400 mt-2 font-bold">يكفي لاستهلاك 35 يوماً للقطيع</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-400 text-xs font-medium">القيمة المالية للمخزون</span>
            <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-xl">
              <DollarSign className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-extrabold text-emerald-400">{formatMoney(totalInventoryValue)}</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-2">تسعير الوارد أولاً صادر أولاً (FIFO)</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-400 text-xs font-medium">متوسط تكلفة طن العليقة TMR</span>
            <div className="p-2 bg-purple-500/10 text-purple-400 rounded-xl">
              <Calculator className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-extrabold text-white">{averageDistributedCostPerTon == null ? 'غير متاح' : formatMoney(averageDistributedCostPerTon)}</span>
            <span className="text-xs text-slate-400">/ طن</span>
          </div>
          <p className="text-[11px] text-purple-400 mt-2 font-bold">محسوب من عمليات الصرف المسجلة فعلياً</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-400 text-xs font-medium">تنبيهات حد الأمان للمخازن</span>
            <div className={`p-2 rounded-xl ${lowStockCount > 0 ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
              <AlertTriangle className="w-5 h-5" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className={`text-3xl font-extrabold ${lowStockCount > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
              {lowStockCount}
            </span>
            <span className="text-xs text-slate-400">نواقص تحت الطلب</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-400">اختر الفئة الإنتاجية أو أدخل نسبة البروتين المستهدفة وحجم خلطة الـ TMR</p>
            </div>

            {/* Target Presets */}
            <div className="space-y-2">
              <label className="block text-xs font-semibold text-slate-300">القطاع والهدف الإنتاجي:</label>
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-2xl border text-right transition ${
                        isSelected 
                          ? 'bg-amber-500/10 border-amber-500 text-white shadow-lg shadow-amber-950/40' 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-3 text-lg font-black text-amber-400 focus:outline-none transition shadow-inner"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 focus:border-amber-500 rounded-xl px-4 py-3 text-lg font-black text-white focus:outline-none transition shadow-inner"
                />
              </div>
            </div>
                  ...
                  return (
                    <label
                      ...
                      className={`p-3 rounded-2xl border flex items-center justify-between cursor-pointer transition ${
                        isChecked 
                          ? 'bg-slate-950/80 border-amber-500/50 text-white' 
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-400">
                            بروتين: <strong className="text-amber-400">{item.proteinPct}%</strong> • رصيد: {Number(item.currentStock).toLocaleString()} كجم
                          </div>
                        </div>
                      </div>

                      <div className="text-left font-bold text-xs text-emerald-400">
                ...
                <div className="space-y-5">
                  ...
                  <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-2xl space-y-2">
                    <div className="flex items-baseline justify-between">
                      <span className="text-xs text-slate-400">تكلفة طن الخلطة:</span>
                      <span className="text-2xl font-black text-emerald-400">{formatMoney(optimizationResult.costPerTon)}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-slate-400 pt-1 border-t border-slate-800/80">
                      <span>إجمالي خلطة الخلاط ({formatNumber(optimizationResult.batchTotalKg)} كجم):</span>
                      <strong className="text-white font-bold">{formatMoney(optimizationResult.totalCost)}</strong>
                    </div>
                    <div className="flex items-center justify-between text-xs text-slate-400">
                      <span>البروتين المحقق:</span>
                      <strong className="text-amber-400 font-bold">{optimizationResult.actualProteinPct}% CP</strong>
                    </div>
                  </div>

                  {/* Batch Weight Breakdown Table */}
                  <div className="space-y-2">
                    <span className="text-xs font-bold text-slate-300 block">أوزان الإضافة في خلاط الـ TMR:</span>
                    <div className="space-y-2">
                      {optimizationResult.items.map(item => (
                        <div key={item.ingredientId} className="p-3 bg-slate-950/50 border border-slate-800 rounded-xl flex items-center justify-between text-xs">
                          <div>
                            <strong className="text-white block">{item.name}</strong>
                            <span className="text-[10px] text-amber-400 font-semibold">{item.percentage}% من الخلطة</span>
                          </div>
                          <div className="text-left">
                            <span className="text-emerald-400 font-extrabold text-sm">{formatNumber(item.weightKg)} كجم</span>
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-slate-100 rounded-xl font-bold text-xs flex items-center justify-center gap-2 border border-slate-700 transition"
                    >
                      <Save className="w-4 h-4 text-emerald-400" />
                      حفظ الخلطة في سجل التركيبات المعتمدة
                    </button>
                    {formulaSaveSuccess && (
                      <p className="text-emerald-400 text-[11px] text-center mt-2 font-bold flex items-center justify-center gap-1">
      ...
      {activeTab === 'warehouse' && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-white text-base">مستودع خامات ومكونات الأعلاف</h3>
              <p className="text-xs text-slate-400">متابعة أرصدة المخزون، أسعار الشراء، وتوريد الشحنات الجديدة</p>
            </div>
            <button
              onClick={() => setIsAddIngredientOpen(true)}
                ...
                {stock.map(item => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="py-3 px-3 font-bold text-amber-400">{formatNumber(item.currentStock)} كجم</td>
                      <td className="py-3 px-3 font-semibold text-slate-200">{formatMoney(item.costPerUnit, true, 3)}</td>
                      <td className="py-3 px-3 font-bold text-emerald-400">{formatMoney(totalVal)}</td>
                      ...
                      <td className="py-3 px-3">
                        {isLow ? (
                          ...
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 w-fit">
                            متوفر آمن
                          </span>
                        )}
                      </td>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {formulas.map(formula => (
              <div key={formula.id} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="font-extrabold text-base text-white">{formula.name}</h4>
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-500/10 border border-purple-500/30 text-purple-400">
                    {formula.targetSector === 'DAIRY' ? 'أبقار حلب' : 'تسمين'}
                  </span>
                </div>

                <p className="text-xs text-slate-400">{formula.description || 'تركيبة علفية TMR معتمدة'}</p>

                <div className="space-y-1.5 text-xs">
                  <span className="font-bold text-slate-300 block mb-1">المكونات ونسب الخلط:</span>
                  {formula.items?.map(item => (
                    <div key={item.id} className="flex justify-between p-2 bg-slate-950/60 rounded-xl border border-slate-800/80">
                      <span className="text-slate-300">{item.ingredient?.name || 'مادة علفية'}</span>
                      <strong className="text-amber-400 font-bold">{item.percentage}%</strong>
                    </div>
                  ))}
                </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
            <h4 className="font-bold text-white text-sm">سجل آخر حركات صرف الأعلاف للحظائر</h4>
            <div className="overflow-x-auto">
              <table className="w-full text-right border-collapse">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-400 text-xs">
                  ...
                  {distributions.length > 0 ? (
                    distributions.map(d => (
                      <tr key={d.id} className="hover:bg-slate-800/40 transition">
                        <td className="py-3 px-3 text-slate-300">{formatDate(d.dispenseDate)}</td>
                        <td className="py-3 px-3 font-bold text-white">{d.barn?.name || 'حظيرة غير متاحة'}</td>
                        <td className="py-3 px-3 text-slate-300">{d.formula?.name || 'خلطة غير متاحة'}</td>
                        <td className="py-3 px-3 font-bold text-amber-400">{formatNumber(d.quantityKg)} كجم</td>
                        <td className="py-3 px-3 font-bold text-emerald-400">{formatMoney(d.totalCost)}</td>
                      </tr>
                    ))
                  ) : (
